...
public class TestController : NetworkBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (Runner.SimulationTime - lastBasicPunchTime >= basicPunchTime && CanStartPunch())
            {
                leftPunchScript.Use();
                anim.SetTrigger("basicPunch");
                lastBasicPunchTime = Runner.SimulationTime; 
                leftPunchStartTime = Runner.SimulationTime;
                anim.SetBool("rightPunch", false);
                isRightPunchActive = false;
            }
            else if (!isRightPunchActive && Runner.SimulationTime - leftPunchStartTime > basicPunchTime - 0.3f && Runner.SimulationTime - leftPunchStartTime < basicPunchTime)
            {
                anim.SetBool("rightPunch", true);
                rightPunchScript.Use();
                rightPunchStartTime = Runner.SimulationTime;
                isRightPunchActive = true;
                isUpperCutActive = false;
                StartCoroutine(SetRightPunchFalseAfterAnimation()); // 코루틴 호출
            }
            else if (!isUpperCutActive && Runner.SimulationTime - rightPunchStartTime > rightPunchTime - 0.3f && Runner.SimulationTime - rightPunchStartTime < rightPunchTime)
            {
        ...
        {
            yield return new WaitForSecondsRealtime(rightPunchTime); // 후속 펀치 애니메이션의 지속 시간
            anim.SetBool("rightPunch", false);
        }

        IEnumerator SetUpperCutFalseAfterAnimation()
        {
            yield return new WaitForSecondsRealtime(upperCutTime);
        ...
        {
            if (Runner.SimulationTime - lastLeftKickTime >= leftKickTime && CanStartPunch())
            {
                leftKickScript.Use();
                anim.SetTrigger("leftKick");
                lastLeftKickTime = Runner.SimulationTime; 
                leftKickStartTime = Runner.SimulationTime;
                anim.SetBool("rightKick", false);
                isRightKickActive = false;
            }
            else if (!isRightKickActive && Runner.SimulationTime - leftKickStartTime > leftKickTime - 0.3f && Runner.SimulationTime - leftKickStartTime < leftKickTime)
            {
                anim.SetBool("rightKick", true);
                rightKickScript.Use();
                rightKickStartTime = Runner.SimulationTime;
        ...
        {
            yield return new WaitForSecondsRealtime(rightKickTime); // 후속 펀치 애니메이션의 지속 시간
            anim.SetBool("rightKick", false);
        }
        ...
        if (Input.GetKeyDown(KeyCode.G) && !isEnd && !hitByUpperCut && Runner.SimulationTime - lastGuardTime >= guardTime && !IsGuardAnimationPlaying() && !IsBasicPunchAnimationPlaying() && !IsLeftKickAnimationPlaying() && !IsRightKickAnimationPlaying() && !IsUnderKickAnimationPlaying() && !IsBasicPunchedAnimationPlaying() && !IsKickedAnimationPlaying() && !IsUnderKickedAnimationPlaying())
        {
            /*rightKickScript.Use();*/
            gFlag = true;
            StartCoroutine(ActivateGuard(0.3f));
            anim.SetTrigger("guard");
            lastGuardTime = Runner.SimulationTime;
        }
    }
    ...
    private void MoveForward()
    {
        ...
        {
            float timeSinceLastPress = Runner.SimulationTime - lastPressTime;
            lastPressTime = Runner.SimulationTime;
            ...
            {
                if(!NoDodge() && Runner.SimulationTime - lastDodge > 1)
                {
        ...
        if (Runner.SimulationTime - lastPressTime > doublePressInterval && pressCount > 0)
        {
            pressCount = 0;
        }
        ...
        IEnumerator DodgeCoroutine(float duration)
        {
            ...
            {
                transform.position = Vector3.MoveTowards(transform.position, targetPosition, 5*speed * Runner.DeltaTime);
    ...
    private void MoveBackward()
    {
        ...
        {
            float timeSinceLastPress = Runner.SimulationTime - lastPressTimeBack;
            lastPressTimeBack = Runner.SimulationTime;
            // 회피 동작 조건 체크
            if (timeSinceLastPress < doublePressInterval) 
            {
                pressCountBack++;
        ...
        // 키 눌림 간격이 너무 길면 키 눌림 횟수 리셋
        if (Runner.SimulationTime - lastPressTimeBack > doublePressInterval && pressCountBack > 0)
        {
            pressCountBack = 0;
        }
        IEnumerator DodgeCoroutine(float duration)
