...
public class enemyPlayer_UDP: MonoBehaviour
{
    ...
    void Start()
    {
        ...
        player = GameObject.FindObjectOfType<player1Controller_UDP>();
        maxHealth = 55;
        particle.Stop();
        light.enabled = false;
        guard.Stop();
 ...
 /*   void HandleLocalInput()
    ...
    // 서버로 입력 데이터를 전송
    void SendInputToServer(string action)
    {
  ...
  /*  void healthSync(int damage)
    ...
    {
        //Debug.Log("receivedData and player function called");
    ...
    void MoveForwardBasic()
    {
        Debug.Log("MoveForwardBasic Called");

        if (!isMovingForward)
        {
            anim.SetBool("stepForward", moveDirection.z > 0);
            anim.SetBool("stepBackward", moveDirection.z < 0);
    ...
    void MoveBackwardBasic()
    {
        if (!isMovingBackward)
        {
            anim.SetBool("stepForward", moveDirection.z < 0);
            anim.SetBool("stepBackward", moveDirection.z > 0);
            isMovingBackward = true; // 이동 상태를 활성화
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.tag == "Melee")
        {
            ...
            if (!gFlag && !hitByUpperCut)
            {
                ...
                //ChangeHealth(-leftPunch.damage); //enemy는 따로 서버에 요청을 보내지 않는다. 
